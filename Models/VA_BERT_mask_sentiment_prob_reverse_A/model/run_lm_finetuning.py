...
class TextDataset(Dataset):
    def __init__(self, tokenizer, sentiment_dict, args, file_path="train", sentiment_w_path='sentiment', add_tokens_data='tokens', block_size=512):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            for i in range(len(text)):
                ...
                #prob
                token.extend([1-sentiment_dict[i] if (i in sentiment_dict) else 0 for i in token])
...
def load_and_cache_examples(args, tokenizer, sentiment_dict, evaluate=False, test=False):
    dataset = TextDataset(
        tokenizer,
        sentiment_dict,
        args,
        file_path=args.eval_data_file if evaluate else args.test_data_file if test else args.train_data_file,
        sentiment_w_path = args.sentiment_w_path,
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, sentiment_mask, args) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    probability_matrix=sentiment_mask.float()*args.mlm_probability
    special_tokens_mask = [
    ...
    indices_replaced = DIST(torch.full(labels.shape, 1)).bool() & masked_indices
...
def train(args, train_dataset, model, tokenizer, sentiment_dict):
    """ Train the model """
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            sentiment = batch[:,int(len(batch[0])/2):]#.long()
            batch = batch[:,:int(len(batch[0])/2)].long()
            inputs, labels = mask_tokens(batch, tokenizer, sentiment, args) if args.mlm else (batch, batch)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    results = evaluate(args, model, tokenizer, sentiment_dict, eval_type='val')
                    ...
                    if best_val_loss > results['best_loss']:
                        ...
                        results = evaluate(args, model, tokenizer, sentiment_dict, eval_type='val', save=True)
                        results = evaluate(args, model, tokenizer, sentiment_dict, eval_type='test', save=True)
...
def evaluate(args, model, tokenizer, sentiment_dict, prefix="", eval_type='val', save=False):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_output_dir = args.output_dir

    if eval_type == 'val':
        print('val lace')
        eval_dataset = load_and_cache_examples(args, tokenizer, sentiment_dict, evaluate=True)
    else:
        print('test lace')
        eval_dataset = load_and_cache_examples(args, tokenizer, sentiment_dict, test=True)
...
def main():
    ...
    f = open(args.sentiment_w_path, 'r')
    sentiment_dict = json.load(f)
    f.close()

    d2 = sentiment_dict.copy()
    for i in tqdm(d2.keys()):
        if len(i.strip())!=1:
            nt = tokenizer.encode(i.strip())#[1]
            sentiment_dict[nt[1]] = sentiment_dict[i]
            del sentiment_dict[i]
        else:
            del sentiment_dict[i]
    ...
    if args.do_train:
        ...
        train_dataset = load_and_cache_examples(args, tokenizer, sentiment_dict, evaluate=False)
        model.resize_token_embeddings(len(tokenizer))
        if args.local_rank == 0:
            torch.distributed.barrier()

        global_step, tr_loss = train(args, train_dataset, model, tokenizer, sentiment_dict)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, sentiment_dict, prefix=prefix)
