...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path="train", sentiment_w_path='sentiment', add_tokens_data='tokens', block_size=512):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            #with open(file_path, encoding="utf-8") as f:
                #text = f.read()
            df = pd.read_csv(add_tokens_data,header=None)
            ...
            f = open(sentiment_w_path, 'r')
            x = f.readline()
            f.close()
            sent = x.split(' ')
            tokenizer.add_tokens(sent)
            sentiment_word = set(tokenizer.convert_tokens_to_ids(sent))
            ...
            for i in range(len(text)):
                ...
                token.extend([0 if (i in sentiment_word) else 1 for i in token])
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, sentiment_mask, args) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    probability_matrix.masked_fill_(sentiment_mask.bool(), value=0.0)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    results = evaluate(args, model, tokenizer, eval_type='val')
                    ...
                    if best_val_loss > results['best_loss']:
                        ...
                        results = evaluate(args, model, tokenizer, eval_type='val', save=True)
                        results = evaluate(args, model, tokenizer, eval_type='test', save=True)
                        # Save model checkpoint
                        output_dir = os.path.join(args.output_dir, "{}".format(checkpoint_prefix))#改掉global_step
...
def evaluate(args, model, tokenizer, prefix="", eval_type='val', save=False):
...
def main():
    ...
    parser.add_argument(
        "--config_name", #config檔，包含模型設定參數等等，如與model_name_or_path不一樣才需設定
        default="",
        type=str,
        help="Optional pretrained config name or path if not the same as model_name_or_path",
    )
    parser.add_argument(
        "--tokenizer_name", #同樣，與model_name_or_path不一樣才需設定
        default="",
        type=str,
    ...
    parser.add_argument(
        "--sentiment_w_path",
        type=str,
        default='./wikitext-2/sentiment_cvaw4.txt',
    ...
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type] #設定model, tokenizer
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer) #tr_loss 可以從這寫判斷式
