...
for m in 0.05 0.1 0.15 0.2 0.25 0.3 0.35 0.4 0.45 0.5;
    ...
    for alpha in 0.6 0.8 1; 
        do
        echo ==================================================$m============================================================
		CUDA_VISIBLE_DEVICES=$device python3.6 ./VA_BERT_mask_sentiment_prob_Union_multilabel/model/run_lm_finetuning.py \
								--train_data_file=./data/train"$fold".csv \
								--eval_data_file=./data/val"$fold".csv \
		                        --test_data_file=./data/test"$fold".csv \
								--output_dir=./VA_BERT_mask_sentiment_prob_Union_multilabel/output/fold"$fold"/output"$m"/alpha"$alpha" \
								--model_type=bert \
