...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, add_tokens_data='tokens', file_path="train", block_size=512):
        assert os.path.isfile(file_path)
        directory, filename = os.path.split(file_path)
        cached_features_file = os.path.join(
            directory, args.model_name_or_path + "_cached_lm_" + str(block_size) + "_" + filename + "_fold" + args.fold
        )
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, args) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    probability_matrix = torch.full(labels.shape, 0, dtype=torch.float)
    #embed()
    special_tokens_mask = [
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            model.train()
            outputs = model(inputs, masked_lm_labels=labels, label_V=label_V, label_A=label_A) if args.mlm else model(inputs, labels=labels)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    print('loss now', results['eval_loss'])
                    loss_df = loss_df.append({'epoch':i, 'loss':results['eval_loss']}, ignore_index=True)
                    loss_df.to_csv('./'+args.output_dir+'/val_loss_df.csv')
                    if best_val_loss > results['eval_loss']:
...
def evaluate(args, model, tokenizer, prefix="", eval_type='val', save=False):
    ...
    #for batch in eval_dataloader:
        ...
        all_predict_V += outputs[1].cpu().tolist()
        all_predict_A += outputs[2].cpu().tolist()
    ...
    if save == True:
        result.to_csv('./'+args.output_dir+'/{}_predict.csv'.format(eval_type))
        ...
        with open('./'+args.output_dir+'/{}_evaluation_result.json'.format(eval_type), 'w') as fp:
            json.dump(dict_, fp)
