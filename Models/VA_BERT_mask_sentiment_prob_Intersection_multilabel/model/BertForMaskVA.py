...
class BertVAHead(nn.Module):
    ...
    def forward(self, pooled_output):
        score = self.prediction(pooled_output)
        return score
...
class BertForMaskVA(BertPreTrainedModel):
    ...
    def forward(
        ...
        score = self.cls_VA(pooled_output)
        valence_score = score[:,0]
        arousal_score = score[:,1]
        ...
        outputs = (1-alpha)*masked_lm_loss+alpha*(loss_V+loss_A)
        loss_VA = loss_V+loss_A
        outputs = (outputs, loss_VA) + outputs_VA 
